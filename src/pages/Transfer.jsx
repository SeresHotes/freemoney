...
export default function Transfer() {
  ...
  useEffect(() => {
    ...
    if (editing) {
      ...
      setFromName(outLeg.wallet);
      setToName(inLeg.wallet);
      ...
      setToName(active[1]?.name || active[0]?.name || '');
    }
    setReady(true);
  }, [editing, outLeg, inLeg, active, ready]);

  const from = active.find((w) => w.name === fromName);
  const to = active.find((w) => w.name === toName);
  ...
  const submit = async (e) => {
    ...
    try {
      if (editing) {
        await updateTransfer({ transferId, outWallet: fromName, inWallet: toName, amountOut: out, amountIn: inc, date, time, note: note.trim() });
      } else {
        await addTransfer({ fromWallet: fromName, toWallet: toName, amountOut: out, amountIn: inc, date, time, note: note.trim() });
      }
      navigate('/');
    } catch {
    ...
    <div className="page">
      ...
      <form className="form" onSubmit={submit}>
        <label className="field">
          ...
          <select className="field__input field__input--select" value={fromName} onChange={(e) => setFromName(e.target.value)}>
        ...
        <label className="field">
          ...
          <select className="field__input field__input--select" value={toName} onChange={(e) => setToName(e.target.value)}>
            {active.map((w) => <option key={w.name} value={w.name}>{w.name} ({w.currency})</option>)}
          </select>
        </label>
