...
export default function Interest() {
  ...
  const [walletName, setWalletName] = useState(active[0]?.name || '');
  ...
  const wallet = active.find((w) => w.name === walletName);
      ...
      <form className="form" onSubmit={submit}>
        <label className="field">
          ...
          <select
            ...
            value={walletName}
            onChange={(e) => { setWalletName(e.target.value); setBaseTouched(false); }}
          >
            {active.map((w) => (
