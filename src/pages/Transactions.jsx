...
export default function Transactions() {
  ...
  const renderPairRow = (t) => {
    ...
    if (isDebt) {
      ...
      amountText = sameVal
        ? formatAmount(Math.abs(outLeg.amount), outLeg.currency)
        : `${formatAmount(Math.abs(outLeg?.amount || 0), outLeg?.currency)} → ${formatAmount(Math.abs(inLeg?.amount || 0), inLeg?.currency)}`;
