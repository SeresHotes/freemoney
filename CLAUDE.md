...
- Служебная метка `updatedAt` (LWW-синхронизация) тоже **читаемая**: в таблице
  хранится как ISO-8601 UTC (`2026-09-16T14:30:00.123Z`), а не «сырые» миллисекунды.
  Внутри приложения `updatedAt` остаётся числом мс (`Date.now()`); кодирование
  туда-обратно — `encStamp`/`decStamp` в `src/api/store.js`, без потери точности.
  UTC, а не локальное время, — чтобы метка одинаково парсилась на всех устройствах.
